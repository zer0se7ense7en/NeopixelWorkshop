...
const int button3Pin = 7;     // the number of the 2nd pushbutton pin
...
// variables for button3 which will change:
int button3PushCounter = 0;   // counter for the number of 2nd button presses
int button3State = 0;         // current state of the 2nd button
int lastButton3State = 0;     // previous state of the 2nd 
...
int brightness = 200;         // variable for the current brightness
...
void setup() {
  ...
  stripe.setBrightness(brightness); // set overall brightness
...
void button3StateChangeDetection() {
  // read the pushbutton input pin:
...
void mode3() {                            // CLIMBING AND DESCENDING RAINBOW CYCLING MODE
  ...
  if (checkForDelayPassed(timer3, 100)) { // use checkForDelayPassed function to check if our requested time has already passed
    ...
    for(int i=0; i<counter; i++) {        // cycle to all leds on strip between position 0 and the current value of counter variable
        stripe.setPixelColor(i, stripe.gamma32(stripe.ColorHSV(65536/NUMPIXELS*i)));   // and give it the color, depending on position on the strip
    }
    stripe.show();                        // update the changes to the real strip
  }
}
